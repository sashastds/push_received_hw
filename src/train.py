...
def main():
    ...
    if data is None:
        with open(model_dir_path / 'log.txt', 'w') as f_out:
            f_out.write(f'was unable to read data from given path: {path_to_data} - exiting\n')
        return 
    
    ### calculating historical features and merging it back to data
    data = data.merge(
        calc_history_daily_features(data),
        on = ['user_id', 'push_time'], 
        how = 'left'
    )
    ...
    hist_features = [f for f in data.columns if f.startswith('mean') or f.startswith('count')]
