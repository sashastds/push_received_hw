...
def load_and_preprocess_data(path_to_data: Union[str, Path]) -> Union[pd.DataFrame, None]:
    ...
    if (
        ...
        and
        not str(path_to_data).endswith('.gz')
    ):
...
def calc_history_daily_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for h in tqdm(last_n_hours):
        ...
        gb = data_hist.loc[mask, :].groupby(['user_id', 'push_day_start'])['push_opened'].agg(['mean', 'count'])
    ...
    data_hist.drop(['mean', 'count', 'push_day_start', 'push_opened'], axis = 1, inplace = True)
    ...
    return data_hist.drop_duplicates(['user_id', 'push_time'], keep = 'first').reset_index(drop = True)
...
def predict_by_hours(
    ...
    all_push_hours = list(range(0, 9)) + list(range(13, 24))
    X_h = deepcopy(X)

    predictions = []
    for h in all_push_hours:
        X_h['push_hour'] = h
        predictions.append(model.predict_proba(X_h)[:, 1])

    P = pd.DataFrame(np.vstack(predictions).T)
    P.columns = all_push_hours
    P['best_push_hour'] = [all_push_hours[x] for x in P.values.argmax(axis = 1)]
    if return_best_only:
...
def make_next_day_predictions(
    ...
    data_for_prediction = calc_history_daily_features(
        pd.concat([
            pd.DataFrame({'user_id': users, 'push_time': next_day, 'push_opened': -1}),
            historical_data.loc[
                (historical_data['push_time'].dt.normalize() == last_day) & (historical_data['user_id'].isin(users)),
                ['user_id', 'push_time', 'push_opened']
            ],
        ], axis = 0, ignore_index = True)
    ).query(f'push_time == "{next_day}"').reset_index(drop = True)

    features = lgbm_model.booster_.feature_name()
    data_for_prediction['best_push_hour'] = predict_by_hours(data_for_prediction[features], lgbm_model, return_best_only = True)
